...
from sqlalchemy import create_engine, Column, Integer, String, MetaData, Table, Float, ForeignKey, DateTime
...
# Session to manage transactions
session = Session()
...
class Bill_App:
    def __init__(self, root):
        ...
        item21 = Label(hygine, text="Hand Sanit", font=("Arial Black", 11), bg="#E5B4F3", fg="#6C3483").grid(row=6,
                                                                                                             column=0,
                                                                                                             pady=11)
        ...
        button_export = Button(button_frame, text="Export CSV", font=("Arial Black", 15), pady=10, bg="#E5B4F3", fg="#6C3483",
                             width=8, command=lambda: export_data(self)).grid(row=0, column=2, padx=10, pady=6)
...
def billarea(self):
    ...
    bill = Bill(customer_name=self.c_name.get(), contact_number=self.phone.get(), total=self.total_all_bill)

    intro(self)

    if self.nutella.get() != 0:
        self.txtarea.insert(END, f"Nutella\t\t {self.nutella.get()}\t{self.nu}\n")
        order = Order(product='Nutella', qty=self.nutella.get(), price=self.nu)
        bill.orders.append(order)

    if self.noodles.get() != 0:
        self.txtarea.insert(END, f"Noodles\t\t {self.noodles.get()}\t{self.no}\n")
        order = Order(product='Noodles', qty=self.noodles.get(), price=self.no)
        bill.orders.append(order)

    if self.lays.get() != 0:
        self.txtarea.insert(END, f"Lays\t\t {self.lays.get()}\t{self.la}\n")
        order = Order(product='Lays', qty=self.lays.get(), price=self.la)
        bill.orders.append(order)

    if self.oreo.get() != 0:
        self.txtarea.insert(END, f"Oreo\t\t {self.oreo.get()}\t{self.ore}\n")
        order = Order(product='Oreo', qty=self.oreo.get(), price=self.ore)
        bill.orders.append(order)

    if self.muffin.get() != 0:
        self.txtarea.insert(END, f"Muffins\t\t {self.muffin.get()}\t{self.mu}\n")
        order = Order(product='Muffins', qty=self.muffin.get(), price=self.mu)
        bill.orders.append(order)

    if self.silk.get() != 0:
        self.txtarea.insert(END, f"Silk\t\t {self.silk.get()}\t{self.si}\n")
        order = Order(product='Silk', qty=self.silk.get(), price=self.si)
        bill.orders.append(order)

    if self.namkeen.get() != 0:
        self.txtarea.insert(END, f"Namkeen\t\t {self.namkeen.get()}\t{self.na}\n")
        order = Order(product='Namkeen', qty=self.namkeen.get(), price=self.na)
        bill.orders.append(order)

    if self.atta.get() != 0:
        self.txtarea.insert(END, f"Atta\t\t {self.atta.get()}\t{self.at}\n")
        order = Order(product='Atta', qty=self.atta.get(), price=self.at)
        bill.orders.append(order)

    if self.pasta.get() != 0:
        self.txtarea.insert(END, f"Pasta\t\t {self.pasta.get()}\t{self.pa}\n")
        order = Order(product='Pasta', qty=self.pasta.get(), price=self.pa)
        bill.orders.append(order)

    if self.rice.get() != 0:
        self.txtarea.insert(END, f"Rice\t\t {self.rice.get()}\t{self.ri}\n")
        order = Order(product='Rice', qty=self.rice.get(), price=self.ri)
        bill.orders.append(order)

    if self.oil.get() != 0:
        self.txtarea.insert(END, f"Oil\t\t {self.oil.get()}\t{self.oi}\n")
        order = Order(product='Oil', qty=self.oil.get(), price=self.oi)
        bill.orders.append(order)

    if self.sugar.get() != 0:
        self.txtarea.insert(END, f"Sugar\t\t {self.sugar.get()}\t{self.su}\n")
        order = Order(product='Sugar', qty=self.sugar.get(), price=self.su)
        bill.orders.append(order)

    if self.dal.get() != 0:
        self.txtarea.insert(END, f"Daal\t\t {self.dal.get()}\t{self.da}\n")
        order = Order(product='Daal', qty=self.dal.get(), price=self.da)
        bill.orders.append(order)

    if self.tea.get() != 0:
        self.txtarea.insert(END, f"Tea\t\t {self.tea.get()}\t{self.te}\n")
        order = Order(product='Tea', qty=self.tea.get(), price=self.te)
        bill.orders.append(order)

    if self.soap.get() != 0:
        self.txtarea.insert(END, f"Soap\t\t {self.soap.get()}\t{self.so}\n")
        order = Order(product='Soap', qty=self.soap.get(), price=self.so)
        bill.orders.append(order)

    if self.shampoo.get() != 0:
        self.txtarea.insert(END, f"Shampoo\t\t {self.shampoo.get()}\t{self.sh}\n")
        order = Order(product='Shampoo', qty=self.shampoo.get(), price=self.sh)
        bill.orders.append(order)

    if self.lotion.get() != 0:
        self.txtarea.insert(END, f"Lotion\t\t {self.lotion.get()}\t{self.lo}\n")
        order = Order(product='Lotion', qty=self.lotion.get(), price=self.lo)
        bill.orders.append(order)

    if self.cream.get() != 0:
        self.txtarea.insert(END, f"Cream\t\t {self.cream.get()}\t{self.cr}\n")
        order = Order(product='Cream', qty=self.cream.get(), price=self.cr)
        bill.orders.append(order)

    if self.foam.get() != 0:
        self.txtarea.insert(END, f"Foam\t\t {self.foam.get()}\t{self.fo}\n")
        order = Order(product='Foam', qty=self.foam.get(), price=self.fo)
        bill.orders.append(order)

    if self.mask.get() != 0:
        self.txtarea.insert(END, f"Mask\t\t {self.mask.get()}\t{self.ma}\n")
        order = Order(product='Mask', qty=self.mask.get(), price=self.ma)
        bill.orders.append(order)

    if self.sanitizer.get() != 0:
        self.txtarea.insert(END, f"Sanitizer\t\t {self.sanitizer.get()}\t{self.sa}\n")
        order = Order(product='Sanitizer', qty=self.sanitizer.get(), price=self.sa)
        bill.orders.append(order)
...
def save_table_to_csv(table_class, filename):
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for record in records:
            writer.writerow([getattr(record, column_name) for column_name in column_names])



def export_data(self):
    save_table_to_csv(Order, "orders.csv")
    save_table_to_csv(Bill, "bills.csv")
